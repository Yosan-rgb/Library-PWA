(function(global) {
  ...
  function resolvePath(base, rel) {
    ...
    for (var i = 0; i < parts.length; i++) {
      if (parts[i] === "..") pathArray.pop();
      else if (parts[i] !== ".") pathArray.push(parts[i]);
    }
  ...
  //custom position format instead of real CFI strings because actual CFI
   //was way too complicated to implement. epos:chapterIndex:scrollFraction
  function encodeCfi(chIdx, frac) {
    return "epos:" + chIdx + ":" + frac.toFixed(5);
  }

  // decode for now. returns null if it's a real epubjs cfi
  function decodeCfi(cfi) {
    if (!cfi) return null;
    var matchResult = cfi.match(/^epos:(\d+):([\d.]+)$/);
    if (matchResult) return { idx: parseInt(matchResult[1]), frac: parseFloat(matchResult[2]) } ;
    return null; }
  ...
  function highlightText(doc, text) {
    ...
    ///soures used interploated from or lifted
  ...
  global.ePub = function(arrayBuffer) {
    ...
    book.ready = JSZip.loadAsync(arrayBuffer).then(async function(zip) {
      ...
      var containerDoc = parseXml(containerXml);
       var rootFile = containerDoc.querySelector("[full-path]");
       var opfPath = rootFile.getAttribute("full-path");
      console.log("Found OPF path at: " + opfPath);
  ...
  function Rendition(book, elementId, options) {
    ...
    function buildIframe() {
      ...
      iframe.scrolling = scrollMode ? "yes" : "no"; 
    ...
    function iDoc() {
      ...
      //grab the book's css so it doesn't look completely unstyled
    ...
    async function loadAll(startFrac) {
      ...
      var doc = iDoc();
      if (!doc) { console.warn("iDoc null, aborting loadAll"); return; }
      doc.open();
      ...
      if (startFrac > 0) {  
        ...
        docEl.scrollTop = startFrac * Math.max(1, docEl.scrollHeight - docEl.clientHeight); 
     }
    ...
    // using CSS columns for page by page - translating the container is the only
    // thing that worked reliably on safari, scrollLeft was broken
    function makeHtml(bodyContent, bookCss, viewW, viewH) {
      var overrides =
        "body{margin:0!important;padding:0!important;" + (scrollMode ? "" : "overflow:hidden!important;") + "background:" + bgColor + "!important;}" +
    ...
    function countPages(doc, viewW) {
      ...
      pageCount = rw ? Math.max(1, Math.ceil(rw.scrollWidth / w)) : 1;
    }
    ...
    return {
      flow: function(mode) {
        var wasScroll = scrollMode;
        var savedChIdx = chIdx; 
        
        scrollMode = (mode === "scrolled-doc");

        if (wasScroll !== scrollMode) {
          buildIframe();
        }
        if (!book._chapters.length) return;
        if (scrollMode) {
          ...
          loadAll(0).then(function() {
            var doc = iDoc();
            if (!doc) return;
            var section = doc.getElementById("ch-" + savedChIdx);
      ...
      display: function(cfi) {
        return book.ready.then(function() {
          if (scrollMode) {
            var pos = decodeCfi(cfi);
            if (pos && pos.frac > 0) {
              return loadAll(0).then(function() {
                setTimeout(function() {
                  var wrapper = document.getElementById("viewer-wrapper");
                  if (wrapper) wrapper.scrollTop = pos.frac * Math.max(1, wrapper.scrollHeight - wrapper.clientHeight);
                }, 350);
              });
            }
            var frac = 0;
      ...
      next: function() {
        ...
        if (scrollMode) {
          ...
          page++;
          goToPage(doc, page, window.innerWidth);
          fireRelocated(doc);
      ...
      annotations: {
        highlight: function(cfi, savedText) {
          var doc = iDoc();
          if (!doc) return;
          try {
            if (savedText) {
              var m = highlightText(doc, savedText);
              if (m) m.setAttribute("data-cfi", cfi);
              savedHighlights[cfi] = { text: savedText, chIdx: chIdx };
              return;
            }
