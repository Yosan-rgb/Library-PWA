...
window.addToLibrary = async function() {
  ...
  if (!file.name.toLowerCase().endsWith(".epub")) {
    showToast("Wroing file type1 Please upload an EPUB");
    fileInput.value = "";
    return;
  }
...
async function setStatus(msg, detail, pct) {
  ...
  try {
    ...
    await setStatus("Reading PDF…", file.name, 5);
    var arrayBuffer = await file.arrayBuffer();
    var pdf = await pdfjsLib.getDocument({ data: arrayBuffer }).promise;
    var totalPages = pdf.numPages;

    await setStatus("Extracting text…", totalPages + " pages found", 10);
    ...
    for (var i = 1; i <= totalPages; i++) {
      ...
      // sort PDF text items into reading order top to bottomleft to right
// https://mozilla.github.io/pdf.js/examples/
function sortByReadingOrder(a, b) {
  var yDiff = b.y - a.y;
  var onSameLine = Math.abs(yDiff) < (a.height * 0.5);
  if (onSameLine) return a.x - b.x;
  return yDiff;
}
      ...
      if (i % 10 === 0) await setStatus("Extracting text…", "Page " + i + " of " + totalPages, pct);
    }

     await setStatus("Detecting chapters…", "", 62);
    ...
    await setStatus("Building EPUB…", chapters.length + " chapters found", 75);

    var title = file.name.replace(/\.pdf$/i, "").replace(/[-_]/g, " ").trim();
    var epubBlob = await makeEpub(title, chapters);

    await setStatus("Adding to library…", "", 90);
...
//these are the patterns that actually show up in real books. might vary and need upgrading. partially my idea need to add source here and appenix
  var chapterPatterns = [
